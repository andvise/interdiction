...
class Instance:
    ...
    def write_dzn_file_flow(self, name):
        file = open(name, "w+")
        file.write("N = " + str(self.N) + ";\n")
        file.write("M = " + str(self.a) + ";\n")
        file.write("R0 = " + str(self.r0) + ";\n")
        ...
        # Edge Start
        file.write("Edge =  [|" + (" ,\n         | ".join(str(x.start) + " ," + str(x.end) for x in self.arcs)) + "|];\n\n")

        # L
        file.write("L = [" + (" ,".join(str(x.length) for x in self.arcs)) + "];\n")

        # D
        file.write("D = [" + (" ,".join(str(x.disruption) for x in self.arcs)) + "];\n")

        # Cost
        file.write("R = [" + (" ,".join(str(x.cost) for x in self.arcs)) + "];\n")

        # Balance
        balance = [0] * self.N
        balance[0] = 1
        balance[self.N-1] = -1
        file.write("Balance = [" + (" ,".join(str(x) for x in balance)) + "];\n")

        file.close()


class InstanceCompetitors:
    m = 10
    n = 10
    N = n * m + 2
    K = 1000
