...
class ORToolsCPMultifollower:

    name = "ORToolsCPMultifollower"
                ...
                if i == end_path[j]:
                    ...
                    for e in range(num_edges):
                        if edge_start[e] == i:
                            ...
                            temp.append(t)
                    model.Add(pi[i][j] == model.Min(temp))
        model.Add(r0 >= model.Sum(x[e] * r[e] for e in range(num_edges)))

        variables = x

        objective = model.Maximize(model.Sum(pi[start_path[j]][j] for j in range(num_paths)), 1)

        solution = model.Assignment()

        solution.Add([pi[start_path[j]][j] for j in range(num_paths)])
        collector = model.LastSolutionCollector(solution)

        model.Solve(model.Phase(variables, model.CHOOSE_FIRST_UNBOUND, model.ASSIGN_MAX_VALUE), [objective, collector])

        for j in range(0, collector.SolutionCount()):
            for i in range(num_paths):
                print('Paths ', i, ' length ', collector.Value(j, pi[start_path[i]][i]))
